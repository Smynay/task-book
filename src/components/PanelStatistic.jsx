...
const PanelStatistic = () => {
  ...
  return (
    // TODO: по БЕМ сначала идет класс элемента, затем миксы. Нет родителя tasks-statistics
    <div className="panel-tasks__statistics panel">
      <div className="tasks-statistics__title panel__title">
        Успехи за неделю
      </div>
      <div className="tasks-statistics__items">
        {taskStatistic &&
          taskStatistic.map(({ name = 'Что-то пошло не так', value = 0 }) => (
            <div key={name} className="tasks-statistics__item">
